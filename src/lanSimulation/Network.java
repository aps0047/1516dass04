...
@return Anwer #true when the broadcast operation was succesful and #false otherwise
	...
	public boolean requestBroadcast(Writer report) {
		...
		do {
			try {
				loggingNodo(report, currentNode);
			} catch (IOException exc) {
				// just ignore
			};
			currentNode = currentNode.nextNode_;
	...
	private void loggingNodo(Writer report, Node currentNode) throws IOException {
		report.write("\tNode '");
...
@return Anwer #true when the print operation was succesful and #false otherwise
				...
				if (document.message_.startsWith("!PS")) {
					...
					if (startPos >= 0) {
						...
						if (startPos >= 0) {
							...
							imprimeDatos(report, author, title);
	...
	private void imprimeDatos(Writer report, String author, String title) throws IOException {
